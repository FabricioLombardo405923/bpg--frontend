...
async function cargarFavoritos() {
    ...
    try {
        const response = await fetch(`${API_BASE}/favoritos/${favoritosState.userId}`);
...
async function eliminarFavorito(idSteam) {
    try {
        const response = await fetch(`${API_BASE}/favoritos/${favoritosState.userId}/${idSteam}`, {
        method: 'DELETE'
        });
...
async function verificarFavorito(idSteam) {
    try {
        const response = await fetch(`${API_BASE}/favoritos/${favoritosState.userId}/check/${idSteam}`);
        const result = await response.json();
...
function getUserId() {
  // Obtener de Firebase Auth o sessionStorage
  // Ajusta esto según tu implementación de auth
    return sessionStorage.getItem('userId') || localStorage.getItem('userId') || 'demo-user-123';
}
